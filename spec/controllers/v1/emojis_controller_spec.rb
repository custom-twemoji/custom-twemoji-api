...
RSpec.describe EmojisController do
  ...
  it 'returns image/png when requested with file_format=png and output=image' do
    ...
    post '/v1/emojis?file_format=png&output=image&renderer=imagemagick', '[{"emoji":"1f600","layers":0}]',
         { 'CONTENT_TYPE' => 'application/json' }

    expect(last_response.status).to eq(200)
    expect(last_response.headers['Content-Type']).to match(%r{image/png})
  end
  ...
  it 'returns an svg image when output=image and file_format=svg' do
    ...
    post '/v1/emojis?file_format=svg&output=image', '[{"emoji":"1f600","layers":0}]',
         { 'CONTENT_TYPE' => 'application/json' }

    expect(last_response.status).to eq(200)
    expect(last_response.headers['Content-Type']).to match(%r{image/svg\+xml})
  ...
  it 'returns a downloadable svg when output=download and file_format=svg' do
    ...
    post '/v1/emojis?file_format=svg&output=download&filename=custom-name', '[{"emoji":"1f600","layers":0}]',
         { 'CONTENT_TYPE' => 'application/json' }
  ...
  it 'returns Base64 encoded png in JSON using canvg renderer' do
    ...
    post '/v1/emojis?file_format=png&renderer=canvg', '[{"emoji":"1f600","layers":0}]',
         { 'CONTENT_TYPE' => 'application/json' }
  ...
  it 'returns image content/html when output=image and file_format=png uses default canvg renderer' do
    ...
    post '/v1/emojis?file_format=png&output=image', '[{"emoji":"1f600","layers":0}]',
         { 'CONTENT_TYPE' => 'application/json' }
